...
- [ ] 1A.1 — Define Pi RPC event types in `packages/contracts/` (agent_start/end, turn_start/end, message_start/update/end, tool_execution_start/update/end, auto_compaction, auto_retry, extension_ui_request)
- [ ] 1A.2 — Define Pi RPC command types in `packages/contracts/` (prompt, steer, follow_up, abort, set_model, set_thinking_level, get_state, get_messages, compact, new_session, switch_session, fork, get_available_models, extension_ui_response)
- [ ] 1A.3 — Define Pi RPC response type in `packages/contracts/` (type, command, success, error, id, data)
